                      ...
                      apiInfo: {
                        ...
                        queryNature: { Add: "INSERT", Update: "UPDATE", View: "SELECT", Delete: "DELETE", List: "SELECT" },
                          ...
                          queryPayload: {
                            ...
                            List: async(req, decryptedPayload) => { return "SELECT COUNT(*) OVER () AS table_count, courses.course_id as courses_id, courses.course_id as id, courses.course_id as courses_courseId,courses.course_sub_domain_prefix as courses_courseSubDomainPrefix,courses.planned_course_id as courses_plannedCourseId,courses.cloid as courses_cloid,courses.teacher_employee_id as courses_teacherEmployeeId,courses.tassist_employee_id as courses_tassistEmployeeId, plannedcourses.course_name as plannedcourses_courseName FROM courses LEFT JOIN plannedcourses ON courses.planned_course_id = plannedcourses.planned_course_id Where courses.status != 'inactive' "},
                            View: async(req, decryptedPayload) => { return "SELECT courses.course_id as courses_id, courses.course_id as id, courses.course_id as courses_courseId,courses.course_sub_domain_prefix as courses_courseSubDomainPrefix,courses.planned_course_id as courses_plannedCourseId,courses.cloid as courses_cloid,courses.teacher_employee_id as courses_teacherEmployeeId,courses.tassist_employee_id as courses_tassistEmployeeId, plannedcourses.course_name as plannedcourses_courseName FROM courses LEFT JOIN plannedcourses ON courses.planned_course_id = plannedcourses.planned_course_id WHERE course_id = {{id}} OR course_id IS NULL"},
