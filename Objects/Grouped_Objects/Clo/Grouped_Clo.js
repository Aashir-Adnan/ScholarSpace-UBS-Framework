                        ...
                        apiInfo: {
                          ...
                          queryNature: { Add: "INSERT", Update: "UPDATE", View: "SELECT", Delete: "DELETE", List: "SELECT" },
                            queryPayload: {
                              ...
                              SELECT 
                            clo.cloid as clo_id, courses.course_id as courses_id, questions.question_id as questions_id, plannedcourses.planned_course_id as plannedcourses_id, employees.employee_id as employees_id, user_roles_designations_department.user_role_designation_department_id as userRolesDesignationsDepartment_id, roles_designations_department.role_designation_department_id as rolesDesignationsDepartment_id, users.user_id as users_id, designations.designation_id as designations_id, roles.role_id as roles_id, departments.department_id as departments_id,
                        courses.course_id as id,
                       clo.clodomain_name as  courses_cloName, courses.course_id as  courses_courseId, questions.question_id as  questions_questionId, plannedcourses.course_name as  courses_plannedcoursesName, employees.employee_id as  employees_employeeId, user_roles_designations_department.user_role_designation_department_id as  userRolesDesignationsDepartment_userRoleDesignationDepartmentId, roles_designations_department.role_designation_department_id as  rolesDesignationsDepartment_roleDesignationDepartmentId, users.username as  courses_usersName, designations.designation_name as  courses_designationsName, roles.role_name as  courses_rolesName, departments.department_name as  courses_departmentsName,

                          clo.cloid as courses_cloid, courses.course_id as courses_courseId, questions.question_id as courses_questionId, plannedcourses.planned_course_id as courses_plannedCourseId, employees.employee_id as courses_employeeId, user_roles_designations_department.user_role_designation_department_id as courses_userRoleDesignationDepartmentId, roles_designations_department.role_designation_department_id as courses_roleDesignationDepartmentId, users.user_id as courses_userId, designations.designation_id as courses_designationId, roles.role_id as courses_roleId, departments.department_id as courses_departmentId,
                          
                       
                         
                           clo.clonum as clo_clonum, clo.clodomain_name as clo_clodomainName, clo.description as clo_description, 
                        courses.course_sub_domain_prefix as courses_courseSubDomainPrefix, courses.planned_course_id as courses_plannedCourseId, courses.cloid as courses_cloid, courses.teacher_employee_id as courses_teacherEmployeeId, courses.tassist_employee_id as courses_tassistEmployeeId FROM clo LEFT JOIN courses ON courses.cloid = clo.cloid AND courses.status !='inactive' LEFT JOIN questions ON questions.cloid = clo.cloid AND questions.status !='inactive' LEFT JOIN plannedcourses ON plannedcourses.planned_course_id = courses.planned_course_id AND plannedcourses.status !='inactive' LEFT JOIN employees ON employees.employee_id = courses.teacher_employee_id AND employees.status !='inactive' LEFT JOIN user_roles_designations_department ON user_roles_designations_department.user_role_designation_department_id = employees.urdd_id AND user_roles_designations_department.status !='inactive' LEFT JOIN roles_designations_department ON roles_designations_department.role_designation_department_id = user_roles_designations_department.role_designation_department_id AND roles_designations_department.status !='inactive' LEFT JOIN users ON users.user_id = user_roles_designations_department.user_id AND users.status !='inactive' LEFT JOIN designations ON designations.designation_id = roles_designations_department.designation_id AND designations.status !='inactive' LEFT JOIN roles ON roles.role_id = roles_designations_department.role_id AND roles.status !='inactive' LEFT JOIN departments ON (departments.employee_id = employees.employee_id OR departments.department_id = roles_designations_department.department_id) WHERE (clo.cloid = {{id}}  AND  clo.status != 'inactive')
                    ...
                    ,
                    {
                      ...
                      data: {
                       ...
                       clo.clodomain_name as  courses_cloName, courses.course_id as  courses_courseId, questions.question_id as  questions_questionId, plannedcourses.course_name as  courses_plannedcoursesName, employees.employee_id as  employees_employeeId, user_roles_designations_department.user_role_designation_department_id as  userRolesDesignationsDepartment_userRoleDesignationDepartmentId, roles_designations_department.role_designation_department_id as  rolesDesignationsDepartment_roleDesignationDepartmentId, users.username as  courses_usersName, designations.designation_name as  courses_designationsName, roles.role_name as  courses_rolesName, departments.department_name as  courses_departmentsName,
                        ...
                        courses.course_sub_domain_prefix as courses_courseSubDomainPrefix, courses.planned_course_id as courses_plannedCourseId, courses.cloid as courses_cloid, courses.teacher_employee_id as courses_teacherEmployeeId, courses.tassist_employee_id as courses_tassistEmployeeId FROM clo LEFT JOIN courses ON courses.cloid = clo.cloid AND courses.status !='inactive' LEFT JOIN questions ON questions.cloid = clo.cloid AND questions.status !='inactive' LEFT JOIN plannedcourses ON plannedcourses.planned_course_id = courses.planned_course_id AND plannedcourses.status !='inactive' LEFT JOIN employees ON employees.employee_id = courses.teacher_employee_id AND employees.status !='inactive' LEFT JOIN user_roles_designations_department ON user_roles_designations_department.user_role_designation_department_id = employees.urdd_id AND user_roles_designations_department.status !='inactive' LEFT JOIN roles_designations_department ON roles_designations_department.role_designation_department_id = user_roles_designations_department.role_designation_department_id AND roles_designations_department.status !='inactive' LEFT JOIN users ON users.user_id = user_roles_designations_department.user_id AND users.status !='inactive' LEFT JOIN designations ON designations.designation_id = roles_designations_department.designation_id AND designations.status !='inactive' LEFT JOIN roles ON roles.role_id = roles_designations_department.role_id AND roles.status !='inactive' LEFT JOIN departments ON (departments.employee_id = employees.employee_id OR departments.department_id = roles_designations_department.department_id) WHERE (questions.question_id = {{id}}  AND  questions.status != 'inactive')
